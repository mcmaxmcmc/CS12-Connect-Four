...
%macro reset 1					;sets [1] to 0 if [2] is 0, increments [1] if [2] is not 0

	;1: The Accused
	;2: Decision -- True (!=0) or False (0) -- from RBX
...
%macro check_pos 5				;Find whether a piece in the specified position is there
	...
	;sub rax, qword[ %2 ]		;41 - 0-41 = 41-0
	
	;mov rbx, qword[ %1 ]		;Quadword position (1-42)

	;mov cl, al
	;shl rbx, cl		;moves the position in question to the leftmost bit (42nd bit) -- 1-42 + 41-0 = 42
	
	;add rax, qword[ %2 ]	;41-0 + 0-41 = 41
	
	;mov cl, al
	;shr rbx, cl		;moves the bit back to the 1st position -- now rbx will be storing either a 1 or 0
...
%macro place 6		;Puts a piece on the board
	...
	;mov rax, qword[player1]
;	call printBorder
;	call printRAX
;	mov rax, qword[player2]
;	call printBorder
;	call printRAX
;	xor rax, rax
;	mov al, byte[ %6 ]
...
;1st bit is bottom left, 42nd bit is top right
player1 dq 0
player1_dump dq 0
player2 dq 0
player2_dump dq 0
		...
		print:
			...
			print_row:
				...
				print_line:
					...
					;mov qword[counter], 0
					;push rax
					;mov rax, 0x2AAAAAAAAAA
					;call printRAX
					;mov qword[player1], rax
					
					;mov rax, 0x15555555555
					;mov qword[player2], rax
					;call printBorder
					;call printBorder
					;call printBorder
				...
				;other stuff like the border of the game goes here -- done
				call printSpace
				call printBorder
			...
			;stuff about printing the vertical spacing between the pieces here -- done
			;call printBorder
			call printBorder
			push rax
			push rcx
			...
			cmp rbx, 0
			jge print
		;stuff about printing the bottom of the board here -- done
...
;		pop rax
		...
		jmp ending		;Jumps to win if value isn't 0, 4, or 5
...
;		call printEndl
		...
		mov qword[counter], 0
		check_TLBR player1, player_input, width, height, counter		;FUCKED
		cmp qword[counter], 0
		jne game_complete1
...
;		call printEndl
		...
		mov qword[counter], 0
		check_tie player1, player2, width, height, counter				;THE PROBLEM
		cmp qword[counter], 0
		jne game_tied1
...
;		call printEndl
		...
		player2_board_check:		;Game board check -- player_input has the position of the newest piece
		;1: quadword of piece positions of a player
	;2: Valid position of piece being checked
	;3: Game board width
	;4: Game board height
...
;Plan:
; - have a way to print each piece in the game board
; 6 rows by 7 columns
; find a way to store the game board -- byte arrays?  each piece takes 2 bits to store
; ------ 2 quadwords
; Have a way to check, at each new piece placed
